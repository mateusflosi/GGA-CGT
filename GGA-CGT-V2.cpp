/************************************************************************************************************************
 ...
 Output:                                                                                                                *
    ...
    configuration i in the input, stored in directory: Solutions_GGA-CGT-V2;                                               *
   If(save_bestSolution = 1) a set of files HGGA_S_(i)_instance.txt including the                                       *
   obtained solution for each instance, for each configuration i, stored in directory: Details_GGA-CGT-V2;                 *
************************************************************************************************************************/
...
void FirstFit(int individual);                       // First Fit
void WorstFit(int individual);                       // Worst Fit
void BestFit(int individual);                        // Best Fit
...
int validation(int individual, int bin, int bin_100)
{
    if (population[individual][bin].Bin_Fullness == bin_capacity)
        bin_100++;
    if (population[individual][bin].Bin_Fullness < population[i][number_items + 4].Bin_Fullness)
        population[individual][number_items + 4].Bin_Fullness = population[i][bin].Bin_Fullness;
    return bin_100;
}

void calculateFitness(int individual, int total_bins)
{
    for (int a = 0; a < total_bins; a++)
...
int *shuffleItens(int individual)
{
    ...
    for (int a = 0; a < number_items; a++)
    {
        int item_a = rand() % number_items;
        int item_b = rand() % number_items;
        int aux = shuffe[item_a];
...
void NextFit(int individual)
{
    ...
    for (int a = 0; a < number_items; a++)
    {
        ...
        {
            bin_100 = validation(individual, total_bins, bin_100);
            total_bins++;
        }

        population[individual][total_bins].Bin_Fullness += weight[shuffle[a]];
        population[individual][total_bins].L.insert(shuffle[a]);
    }

    bin_100 = validation(individual, total_bins, bin_100);

    calculateFitness(individual, total_bins + 1);
    population[individual][number_items + 1].Bin_Fullness = total_bins + 1;
...
void Next2Fit(int individual)
{
    ...
    for (int a = 0; a < number_items; a++)
    {
        ...
        if (total_bins > 0 && population[individual][total_bins - 1].Bin_Fullness + weight[shuffle[a]] <= bin_capacity)
            b--;
        else if (population[individual][total_bins].Bin_Fullness + weight[shuffle[a]] > bin_capacity)
        {
            if (total_bins > 0)
                bin_100 = validation(individual, total_bins - 1, bin_100);
            total_bins++;
            b++;
        }

        population[individual][b].Bin_Fullness += weight[shuffle[a]];
        population[individual][b].L.insert(shuffle[a]);
    }
...
void FirstFit(int individual)
{
    ...
    for (int a = 0; a < number_items; a++)
    {
        ...
        {
            if (population[individual][b].Bin_Fullness + weight[shuffle[a]] <= bin_capacity)
                break;
        }
        ...
        population[individual][b].Bin_Fullness += weight[shuffle[a]];
        population[individual][b].L.insert(shuffle[a]);
        bin_100 = validation(individual, b, bin_100);
    }

    calculateFitness(individual, total_bins);
    population[individual][number_items + 1].Bin_Fullness = total_bins;
    population[individual][number_items + 3].Bin_Fullness = bin_100;
}

void WorstFit(int individual)
{
    ...
    {
        int index = bins[0];
        if (population[individual][index].Bin_Fullness + weight[shuffle[a]] > bin_capacity)
            index = total_bins;

        population[individual][index].Bin_Fullness += weight[shuffle[a]];
        population[individual][index].L.insert(shuffle[a]);
        bin_100 = validation(individual, index, bin_100);

        int b;
        if (index == bins[0])
        {
            for (b = 1; b < total_bins; b++)
            {
                if (population[individual][bins[b]].Bin_Fullness < population[individual][index].Bin_Fullness)
                    bins[b - 1] = bins[b];
                else
                    break;
            }
...
void BestFit(int individual)
{
    ...
    for (int a = 0; a < number_items; a++)
    {
        int index = total_bins, b;
        for (b = 0; b < total_bins; b++)
        {
            if (population[individual][bins[b]].Bin_Fullness + weight[shuffle[a]] <= bin_capacity)
            {
                index = bins[b];
                break;
            }
        ...
        while (b > 0)
        {
            if (population[individual][index].Bin_Fullness < population[individual][bins[b - 1]].Bin_Fullness)
                break;
            bins[b] = bins[b - 1];
            b--;
        }
...
void sendtofile(SOLUTION best[])
{
    ...
    strcpy(fil, "Details_GGA-CGT-V2/GGA-CGT_S_(");
    strcpy(string1, file);
    // itoa(conf, aux, 10);
...
int main(int argc, char *argv[])
{
    ...
    if (myrank == 0)
    {
        ...
        system("mkdir Solutions_GGA-CGT-V2");
        system("mkdir Details_GGA-CGT-V2");
        ...
        while (fgets(string, 150, input_Configurations))
        {
            ...
            strcpy(nameC, "Solutions_GGA-CGT-V2/GGA-CGT_(");
            snprintf(aux, sizeof(aux), "%ld", conf);
            strcat(nameC, aux);
            strcat(nameC, ".");
